...
def get_aruco_axis(img_L, img_R, aruco_detector, board_coord, cams_params):
    """
    从左右图像中检测ArUco标记并估计其姿态
    Args:
        img_L: 左相机图像
        img_R: 右相机图像 
        aruco_detector: ArUco检测器
        board_coord: ArUco标记板坐标字典
        cams_params: 相机参数元组(mtx_left, dist_left, mtx_right, dist_right)
    Returns:
    ...
    img_coords_L = []  # 存储图像坐标
    point_coords_L = []  # 存储3D点坐标
    aruco_ps_L_2camL = []  # 存储相机坐标系下的点
    ...
    if ids_L is not None:
        for i in range(len(ids_L)):
            ...
            # 获取标记四个角点
            tmp_marker = corners_L[i][0]  # 获取第i个标记的角点

            # 转换为整数坐标用于绘制
            tmp_marker_tl = (int(tmp_marker[0][0]), int(tmp_marker[0][1]))  # 左上
            tmp_marker_tr = (int(tmp_marker[1][0]), int(tmp_marker[1][1]))  # 右上
            tmp_marker_br = (int(tmp_marker[2][0]), int(tmp_marker[2][1]))  # 右下
            tmp_marker_bl = (int(tmp_marker[3][0]), int(tmp_marker[3][1]))  # 左下

            # 在图像上用不同颜色标记四个角点
            cv2.circle(img_L, tmp_marker_tl, 10, (0, 0, 255), -1)     # 红色-左上
            cv2.circle(img_L, tmp_marker_tr, 10, (0, 255, 0), -1)     # 绿色-右上
            cv2.circle(img_L, tmp_marker_br, 10, (255, 0, 0), -1)     # 蓝色-右下
            cv2.circle(img_L, tmp_marker_bl, 10, (0, 170, 255), -1)   # 橙色-左下

            # 构建角点坐标数组
            img_coord = np.array([
                [tmp_marker[0][0], tmp_marker[0][1]],  # 左上角点坐标
                [tmp_marker[1][0], tmp_marker[1][1]],  # 右上角点坐标
                [tmp_marker[2][0], tmp_marker[2][1]],  # 右下角点坐标
                [tmp_marker[3][0], tmp_marker[3][1]]   # 左下角点坐标
            ])
            ## 將2D圖像坐標添加到列表中
            ...
            # 1. 準備2D-3D對應點
            image_C_L = np.ascontiguousarray(img_coord[:,:2]).reshape((-1,1,2))  # 2D圖像坐標
            ret_L, rvec_L, tvec_L = cv2.solvePnP(tem_coord, image_C_L, mtx_left, dist_left)  # 解PnP問題
            #np.hstack() 水平堆疊這兩部分，將2D坐標轉換為3D坐標 (x,y,0

            # 2. 計算旋轉矩陣和平移向量
            R_aruco2camL, _ = cv2.Rodrigues(rvec_L)  # 將旋轉向量轉換為旋轉矩陣
            t_aruco2camL = tvec_L  # 平移向量
## 完整的数学表达式如下:
# P_camera = R * P_aruco + t
# 其中:
# P_camera: 相机坐标系中的点坐标
# P_aruco: ArUco标记坐标系中的点坐标
# R: 旋转矩阵 (3x3)
# t: 平移向量 (3x1)
            # 3. 計算ArUco標記點在相機坐標系中的位置
            aruco_p_L_2camL = np.dot(R_aruco2camL, tem_coord.T).T + t_aruco2camL.T  # 坐標轉換
            # 详细步骤说明:
# 1. tem_coord: [N×3] 矩阵，包含N个ArUco标记点的坐标
# 2. tem_coord.T: [3×N] 矩阵，转置以便进行矩阵乘法
# 3. R_aruco2camL: [3×3] 旋转矩阵
# 4. np.dot(R_aruco2camL, tem_coord.T): [3×N] 矩阵，执行旋转变换
# 5. (...).T: [N×3] 矩阵，转置回原始形状
# 6. t_aruco2camL.T: [1×3] 平移向量
# 7. 最终加上平移向量，完成坐标转换
            aruco_ps_L_2camL.append(aruco_p_L_2camL)  # 保存結果

    # 合并所有检测到的点
    # 添加调试信息
    if not img_coords_L:
        print(f"左相机未检测到任何ArUco标记点")
    else:
        detected_ids = [ids_L[i][0] for i in range(len(ids_L))]
        print(f"左相机检测到的ArUco标记ID: {detected_ids}")
        
    # 合并所有检测到的2D图像坐标点
    img_coords_L = np.concatenate(img_coords_L, axis=0) if img_coords_L else np.array([])
    print(f"检测到的2D坐标点数量: {len(img_coords_L)}")
    ...
    print(f"对应的3D世界坐标点数量: {len(point_coords_L)}")

    # 合并所有转换到左相机坐标系下的ArUco标记点坐标
    aruco_ps_L_2camL = np.concatenate(aruco_ps_L_2camL, axis=0) if aruco_ps_L_2camL else np.array([])
    print(f"转换到相机坐标系的点数量: {len(aruco_ps_L_2camL)}")

    # 如果需要,可以添加一个字典来记录每个标记的检测历史
    detection_history = defaultdict(list)
    if ids_L is not None:
        for i in range(len(ids_L)):
            marker_id = ids_L[i][0]
            detection_history[marker_id].append(True)
    else:
        for marker_id in board_coord.keys():
            detection_history[marker_id].append(False)

    # 在右图像中检测ArUco标记,处理方式与左图像相同
    ...
    # axis=0表示沿著第一個維度(垂直方向)進行拼接
    # 例如: 如果有兩個2D陣列 [[1,2],[3,4]] 和 [[5,6],[7,8]]
    # axis=0拼接結果為 [[1,2],[3,4],[5,6],[7,8]]
    # 將所有右相機的ArUco角點座標進行拼接
    # 例如: 如果img_coords_R包含兩個角點座標陣列 [[1,2],[3,4]] 和 [[5,6],[7,8]]
    # 拼接後會變成 [[1,2],[3,4],[5,6],[7,8]]
    img_coords_R = np.concatenate(img_coords_R, axis=0) if img_coords_R else np.array([])
    
    # 在角點座標最後一列加上1,形成齊次座標
    # 例如: [[1,2],[3,4]] 變成 [[1,2,1],[3,4,1]]
    img_coords_R = np.hstack((img_coords_R, np.ones(len(img_coords_R))[:,None])) if len(img_coords_R) > 0 else np.array([])
    
    # 將所有右相機的ArUco標記3D座標進行拼接
    # 例如: 如果point_coords_R包含兩組3D座標 [[x1,y1,z1],[x2,y2,z2]] 和 [[x3,y3,z3],[x4,y4,z4]]
    # 拼接後會變成 [[x1,y1,z1],[x2,y2,z2],[x3,y3,z3],[x4,y4,z4]]
    point_coords_R = np.concatenate(point_coords_R, axis=0) if point_coords_R else np.array([])
    
    # 將所有右相機座標系下的ArUco點座標進行拼接
    # 同樣是將多組3D座標陣列合併成一個大陣列
...
def process_frame(detector, frame_left, frame_right, img_L, img_R, cams_params, cam_P):
    ...
    处理单帧图像
    Args:
        detector: MediaPipe检测器
        frame_left, frame_right: 原始帧
        img_L, img_R: 用于绘制的图像副本
        cams_params: 相机参数
        cam_P: ArUco外部参数 (R_aruco2camL, t_aruco2camL, R_aruco2camR, t_aruco2camR)
    """
    # 解包相机参数和外部参数
    (mtx_left, dist_left, mtx_right, dist_right) = cams_params
    ...
    # 在左右图像上投影坐标轴
    # 左图像
    image_points_L, _ = cv2.projectPoints(axis_coord, cv2.Rodrigues(R_aruco2camL)[0], t_aruco2camL, mtx_left, dist_left)
    image_points_L = image_points_L.reshape(-1, 2).astype(np.int32)
    # 绘制坐标轴
    cv2.line(img_L, tuple(image_points_L[0]), tuple(image_points_L[1]), (0,0,255), 5) # X轴 红色
    cv2.line(img_L, tuple(image_points_L[0]), tuple(image_points_L[2]), (0,255,0), 5) # Y轴 绿色
    cv2.line(img_L, tuple(image_points_L[0]), tuple(image_points_L[3]), (255,0,0), 5) # Z轴 蓝色
    
    # 右图像
    image_points_R, _ = cv2.projectPoints(axis_coord, cv2.Rodrigues(R_aruco2camR)[0], t_aruco2camR, mtx_right, dist_right)
    image_points_R = image_points_R.reshape(-1, 2).astype(np.int32)
    # 绘制坐标轴
    cv2.line(img_R, tuple(image_points_R[0]), tuple(image_points_R[1]), (0,0,255), 5) # X轴 红色
    cv2.line(img_R, tuple(image_points_R[0]), tuple(image_points_R[2]), (0,255,0), 5) # Y轴 绿色
    cv2.line(img_R, tuple(image_points_R[0]), tuple(image_points_R[3]), (255,0,0), 5) # Z轴 蓝色
...
def transform_bone_points(points_3d, bone_points):
    ...
    rotation_L, translation_L = define_coordinate_system([points_3d[27],points_3d[31],points_3d[29]])
    rotation_R, translation_R = define_coordinate_system([points_3d[28],points_3d[32],points_3d[30]])
...
def process_videos(video_path_left, video_path_right, start_frame=0):
   ...
   # bone_points_L = load_bone_points(r"D:\projects\01jobs\002skeleton\20241025\L_footpoints.csv")
    ...
    # 读取第一帧获取ArUco外部参数
    ret_left, first_frame_left = cap_left.read()
    ret_right, first_frame_right = cap_right.read()
    if not ret_left or not ret_right:
        raise ValueError("无法读取第一帧")
        
    result = get_aruco_axis(first_frame_left, first_frame_right, aruco_detector, board_coord, cams_params)
    if result[0] is None:
        raise ValueError("第一帧未检测到ArUco标记")
    R_aruco2camL, t_aruco2camL, R_aruco2camR, t_aruco2camR, R_camL2aruco, t_camL2aruco, R_camR2aruco, t_camR2aruco, _, _ = result
    ...
    while True:
        ...
        try:
            # 使用第一帧获取的外部参数处理当前帧
            points_3d, img_L, img_R = process_frame(detector, frame_left, frame_right, 
                                                  frame_left.copy(), frame_right.copy(), 
                                                  cams_params, cam_P)
            
            if points_3d is None:
                print(f"第 {frame_count + 1} 帧人体姿态检测失败")
                all_points_3d.append(np.zeros((33, 3)))
                all_bone_points.append(np.zeros((len(bone_points[0]) + len(bone_points[1]), 3)))
                aruco_axis.append(axis_coord)  # 使用预定义的axis_coord
...
def visualize_with_pltAnimation_2(transformed_points_3d, aruco_axises, camL_axises, camR_axises):
    ...
    def update(frame):
        ...
        # 更新坐标系线条
        # aruco坐标系
        skeleton_lines[-9].set_data_3d([aruco_axis[0, 0], aruco_axis[1, 0]],
                                    [aruco_axis[0, 1], aruco_axis[1, 1]],
                                    [aruco_axis[0, 2], aruco_axis[1, 2]])
        skeleton_lines[-8].set_data_3d([aruco_axis[0, 0], aruco_axis[2, 0]],
                                    [aruco_axis[0, 1], aruco_axis[2, 1]],
                                    [aruco_axis[0, 2], aruco_axis[2, 2]])
        skeleton_lines[-7].set_data_3d([aruco_axis[0, 0], aruco_axis[3, 0]],
                                    [aruco_axis[0, 1], aruco_axis[3, 1]],
                                    [aruco_axis[0, 2], aruco_axis[3, 2]])
        # 左相机坐标系
        skeleton_lines[-6].set_data_3d([camL_axis[0, 0], camL_axis[1, 0]],
                                    [camL_axis[0, 1], camL_axis[1, 1]],
                                    [camL_axis[0, 2], camL_axis[1, 2]])
        skeleton_lines[-5].set_data_3d([camL_axis[0, 0], camL_axis[2, 0]],
                                    [camL_axis[0, 1], camL_axis[2, 1]],
                                    [camL_axis[0, 2], camL_axis[2, 2]])
        skeleton_lines[-4].set_data_3d([camL_axis[0, 0], camL_axis[3, 0]],
                                    [camL_axis[0, 1], camL_axis[3, 1]],
                                    [camL_axis[0, 2], camL_axis[3, 2]])
        # 右相机坐标系
        skeleton_lines[-3].set_data_3d([camR_axis[0, 0], camR_axis[1, 0]],
                                    [camR_axis[0, 1], camR_axis[1, 1]],
                                    [camR_axis[0, 2], camR_axis[1, 2]])
        skeleton_lines[-2].set_data_3d([camR_axis[0, 0], camR_axis[2, 0]],
                                    [camR_axis[0, 1], camR_axis[2, 1]],
                                    [camR_axis[0, 2], camR_axis[2, 2]])
        skeleton_lines[-1].set_data_3d([camR_axis[0, 0], camR_axis[3, 0]],
