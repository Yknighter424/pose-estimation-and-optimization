...
for landmark in pose_landmarks_list1[0]:
    a = landmark.x  # 获取地标的x坐标
    b = landmark.y  # 获取地标的y坐标
    pose1[count1, 0] = int(1920 * a)  # 将x坐标转换为像素值并存储
    pose1[count1, 1] = int(1200 * b)  # 将y坐标转换为像素值并存储
    count1 = count1 + 1  # 更新计数器

# 遍历第二个姿态地标列表中的每个地标
for landmark in pose_landmarks_list2[0]:
    a = landmark.x  # 获取地标的x坐标
    b = landmark.y  # 获取地标的y坐标
    pose2[count2, 0] = int(1920 * a)  # 将x坐标转换为像素值并存储
    pose2[count2, 1] = int(1200 * b)  # 将y坐标转换为像素值并存储，1200-1200b貌似在做Y轴的反向
    count2 = count2 + 1  # 更新计数器
...
# 可視化平滑後與GOM優化後的數據比較
    ...
    # objp: 3D棋盤格的對象點
    # nx, ny: 棋盤格的內角點數量
    # square_size: 每個方格的大小（例如，3.0 mm）

    # 設定棋盤格的參數
    square_size = 3.0  # 根據實際棋盤格方格的尺寸修改
    objp = np.zeros((ny * nx, 3), np.float32)
    objp[:, :2] = np.mgrid[0:nx, 0:ny].T.reshape(-1, 2) * square_size  # 將坐標乘以方格大小
    ...
    # 繪製棋盤格的3D點
    ax.scatter(objp[:, 0], objp[:, 1], objp[:, 2], c='b', marker='o')

    # 繪製棋盤格的邊界
    for i in range(ny):
        for j in range(nx):
            if j < nx - 1:  # 繪製水平邊界
                ax.plot([objp[i * nx + j, 0], objp[i * nx + j + 1, 0]],
                        [objp[i * nx + j, 1], objp[i * nx + j + 1, 1]],
                        [objp[i * nx + j, 2], objp[i * nx + j + 1, 2]], c='r')
            if i < ny - 1:  # 繪製垂直邊界
                ax.plot([objp[i * nx + j, 0], objp[(i + 1) * nx + j, 0]],
                        [objp[i * nx + j, 1], objp[(i + 1) * nx + j, 1]],
                        [objp[i * nx + j, 2], objp[(i + 1) * nx + j, 2]], c='r')

    # 設置坐標軸標籤
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('3D Chessboard Points Visualization')

    # 設置坐標軸比例
    ax.set_box_aspect([1, 1, 1])  # 設置坐標軸比例相等

    # 設置坐標軸範圍
    ax.set_xlim([0, nx * square_size])
    ax.set_ylim([0, ny * square_size])
    ax.set_zlim([0, 0])  # Z軸範圍可以設置為0，因為棋盤格在Z=0平面上
