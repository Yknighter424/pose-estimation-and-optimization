...
'''def detect_chessboard_corners(image, pattern_size=(9, 6)):
    ...
    ret, corners = cv2.findChessboardCorners(
        gray, pattern_size, None)#檢測棋盤格角點
    if ret:
        #如果檢測到角點，則進行亞像素級角點精確化 
        criteria = (cv2.TERM_CRITERIA_EPS +
                    cv2.TERM_CRITERIA_MAX_ITER, 100, 1e-6)#迭代終止條件
        corners2 = cv2.cornerSubPix(
            gray, corners, (11, 11), (-1, -1), criteria)#亞像素級角點精確化
...
for idx, (img_left_path, img_center_path, img_right_path) in enumerate(zip(images_left, images_center, images_right)):
    img_left = cv2.imread(img_left_path)#讀取左相機圖像
    img_center = cv2.imread(img_center_path)#讀取中間相機圖像
    img_right = cv2.imread(img_right_path)#讀取右相機圖像

    ret_left, corners_left = detect_chessboard_corners(
        img_left, pattern_size)#檢測左相機棋盤格角點
    ret_center, corners_center = detect_chessboard_corners(
        img_center, pattern_size)#檢測中間相機棋盤格角點
    ret_right, corners_right = detect_chessboard_corners(
        img_right, pattern_size)#檢測右相機棋盤格角點

    if ret_left and ret_center and ret_right:
        objpoints.append(objp)#添加物體點
        imgpoints_left.append(corners_left)#添加左相機角點
        imgpoints_center.append(corners_center)#添加中間相機角點
        imgpoints_right.append(corners_right)#添加右相機角點
    else:
        print(f"无法在第 {idx+1} 组图像中检测到所有角点，跳过此组")
...
imggg3 = cv2.imread(r"D:\20241112\SUBCALI0\subcaliCam0-0.jpeg")    # 右相机图像'path/to/right_image.jpg'
